...
(defmethod slow-window-event-by-name (name)
  (position name #(:none :shown :hidden :exposed :moved :resized
		   :size-changed :minimized :maximized :restored
		   :enter :leave :focus-gained :focus-lost :close
		   :take-focus :hit-test)))

(defmethod slow-mouse-button-by-name (name)
  (position name #(:0 :left :middle :right :other0 :other1 :other2 :other3 :other4)))

(defmethod slow-keyboard-key-by-name (name)
  (position name
